...
<div class="page page--centered page--dark">
    <form class="form" method="post" action="<?= url_to('Admin\Config::save') ?>" autocomplete="off">
        ...
        <div class="form__group form__group--centered">
            ...
            <input type="file" title='Upload image' id="image-uploader" name="image-uploader" accept="image/*" onchange="updateImage(this)" hidden>

            <image id="image" class="form__logo" style="width: 15rem; height: 15rem; object-fit: cover" src="<?= \App\Libraries\Resource::pathToURL($defaultImage) ?>" alt="No image" title="Click to change" onclick="document.getElementById('image-uploader').click()">
            </image>
        ...
        <fieldset class="form__group">
            ...
            <input class="form__input" type="url" id="home_url" name="home_url" pattern="https://.*" placeholder="https://example.com" value="<?= $homeURL ?>" onchange="updateHome(this)" required>
        </fieldset>

        <!-- ABOUT URL -->
        <fieldset class="form__group">
            <label class="form__label" for="about_url">About URL</label>
            <input class="form__input" type="url" id="about_url" name="about_url" pattern="https://.*" placeholder="https://example.com" value="<?= $aboutURL ?>" onchange="updateAbout(this)" required>
        </fieldset>
...
<script>
    ...
    const updateURL = (element, callback) => {
        ...
        processedFetch('<?= url_to('Admin\Config::save') ?>', {
            method: 'POST',
            body
        }, response => {

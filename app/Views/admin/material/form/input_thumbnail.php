...
<div clas="form__group">
    ...
    <!-- thumbnail view -->
    <image id="thumbnail" class="form__logo" style="width: 15rem; height: 15rem; object-fit: cover" src="<?= \App\Libraries\Resource::pathToURL($rootPath) ?>" alt="No image" onclick="document.getElementById('thumbnail-uploader').click()" title="Image is cropped and resized to 512x512; provide an image with 1:1 aspect ratio if possible!">
    </image>

    <input id="thumbnail-path" type="hidden" name="thumbnail" value="<?= $rootPath ?>">

    <!-- file uploader -->
    <input type="file" title='Upload thumbnail' id="thumbnail-uploader" name="thumbnail-uploader" accept="image/*" onchange="uploadThumbnail()" hidden>

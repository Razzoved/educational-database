...
<?php if ($childCount > 0) : ?>
    ...
    <div class="collapsible collapsible--no-overflow<?= (!$isFirstLevel && $type !== 'button') ? ' collapsible--collapsed' : '' ?>">
        ...
        <div class="collapsible__header">
            ...
            <?php if ($type !== 'button') : ?>
                <input class="collapsible__toggle-group" type="checkbox" id="filter_<?= $property->id ?>" name="group" value="<?= $property->id ?>" title="Use all from group" onchange="toggleCollapsible(this, 'collapsible--selected')" />
            <?php else : ?>
                <button class="collapsible__header--search" type="button" onclick="searchCategory(this.closest('.collapsible'))">
                    <input type="hidden" id="filter_<?= $property->id ?>" name="group" value="<?= $property->id ?>" data-action="<?= url_to('Material::index') ?>" />

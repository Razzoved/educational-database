...
<li class="collapsible__item<?= $isOverflow ? ' collapsible__item--overflow' : '' ?>">
    ...
    <?php elseif ($type === 'button') : ?>
        ...
        <form class="filter" method="get" action='<?= url_to('Material::index') ?>'>
            <input type="hidden" id="filter_<?= $property->id ?>" name="filter" value="<?= $property->id ?>">
    ...
    <?php else : ?>
        ...
        <fieldset class="filter" <?= $tooltip ?>>
            <input class="filter__checkbox" type="checkbox" id="filter_<?= $property->id ?>" name="filter" value="<?= $property->id ?>">
            </input>
